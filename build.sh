...
phpImg="${REGISTRY}/dev/bullseye-php83"
phpTag="1.0.0"
phpFpmImg="${REGISTRY}/dev/bullseye-php83-fpm"
phpFpmTag="2.0.0-arm1"
phpJobRunnerImg="${REGISTRY}/dev/bullseye-php83-jobrunner"
...
## Building php8.3 bullseye image
echo "Building php8.3 bullseye image ${phpImg}:${phpTag}"
docker build . -f php83/Dockerfile -t "${phpImg}:${phpTag}"
## Building php8.3 FPM bullseye image
echo "Building php8.3 FPM bullseye image ${phpFpmImg}:${phpFpmTag}"
docker build . -f fpm/Dockerfile -t "${phpFpmImg}:${phpFpmTag}"
## Building php8.3 Job Runner bullseye image
echo "Building php8.3 Job Runner bullseye image ${phpJobRunnerImg}:${phpJobRunnerTag}"
